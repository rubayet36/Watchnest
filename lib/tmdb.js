...
export async function tmdbFetch(endpoint, params = {}, options = {}) {
  ...
  if (!res.ok) throw new Error(data.error || `TMDB error: ${res.status}`)
  return data
}
...
export async function getRecommendations(tmdbId, mediaType = 'movie', options = {}) {
  return tmdbFetch(`/${mediaType}/${tmdbId}/recommendations`, {}, options)
}
